...
CREATE TABLE Film (
    ...
    CONSTRAINT FILM_FK FOREIGN KEY (ID) REFERENCES Programma(ID)
        ON UPDATE CASCADE 
        ON DELETE CASCADE
);
...
CREATE TABLE Aflevering (
    ...
    CONSTRAINT AFLEVERING_FK FOREIGN KEY (ID) REFERENCES Programma(ID)
        ON UPDATE CASCADE 
        ON DELETE CASCADE, 
...
CREATE TABLE Bekeken (
	...
	CONSTRAINT BEKEKEN_FK FOREIGN KEY (Abonneenummer, Profielnaam) REFERENCES Profiel(Abonneenummer, Profielnaam)
        ON UPDATE CASCADE
        ON DELETE CASCADE,
    CONSTRAINT BEKEKEN_FK2 FOREIGN KEY (Gezien) REFERENCES Programma(ID)
        ON UPDATE CASCADE
        ON DELETE CASCADE
);
